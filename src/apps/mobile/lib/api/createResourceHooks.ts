  ...
  function useCreate(): UseMutationResult<T, ApiError, CreateDto> {
    ...
    return useMutation<T, ApiError, CreateDto>({
      ...
      onSuccess: () => {
        ...
        toastBridge.success(copy?.create?.success ?? `${titleCase} created`);
      },
      onError: (err) => {
        toastBridge.error(copy?.create?.error ?? `Could not create ${resource}`, err.message);
      },
    });
  }
  ...
  function useUpdate(): UseMutationResult<T, ApiError, { id: string; patch: UpdateDto }> {
    ...
    return useMutation<T, ApiError, { id: string; patch: UpdateDto }>({
      ...
      onSuccess: (_data, variables) => {
        ...
        toastBridge.success(copy?.update?.success ?? `${titleCase} updated`);
      },
      onError: (err) => {
        toastBridge.error(copy?.update?.error ?? `Could not update ${resource}`, err.message);
      },
    });
  }
  ...
  function useDelete(): UseMutationResult<void, ApiError, string> {
    ...
    return useMutation<void, ApiError, string, DeleteContext>({
      ...
      onError: (err, _id, ctx) => {
        ...
        toastBridge.error(copy?.delete?.error ?? `Could not delete ${resource}`, err.message);
      },
      onSuccess: () => {
        toastBridge.success(copy?.delete?.success ?? `${titleCase} deleted`);
      },
      onSettled: () => {
        qc.invalidateQueries({ queryKey: keys.all });
      },
