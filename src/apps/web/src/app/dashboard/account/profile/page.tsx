...
import { getUserById } from "@/lib/api/adapters/user.adapter";
        ...
        <CardHeader className="flex flex-col md:flex-row items-center gap-6 p-6 bg-muted/20">
            <div className="relative group">
              <Avatar className="h-32 w-32 border-2 border-primary/50 group-hover:border-primary/100 transition-colors shadow-md">
                ...
                <AvatarFallback className="text-4xl">{user.name?.split(' ').map((n: string) => n[0]).join('')}</AvatarFallback>
              </Avatar>
          ...
          <div className="text-center md:text-left">
            ...
            <CardDescription className="text-md text-muted-foreground capitalize">{user.role?.replace(/_/g, ' ')}</CardDescription>
            <p className="text-sm text-muted-foreground mt-1">Joined on: {formatDateStandard(user.createdAt)}</p>
          </div>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Input id="phone" type="tel" value={user.phone || 'N/A'} readOnly className="mt-1 bg-background/50" />
            </div>
          </div>
