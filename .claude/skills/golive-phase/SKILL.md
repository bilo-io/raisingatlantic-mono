---
name: golive-phase
description: Plan and execute a go-live phase end-to-end from the role roadmaps under docs/GO_LIVE/ (DEV.md phases 0–14 or MOBILE.md phases M0–M5). Browse outstanding tasks, plan in plan mode, implement in an isolated worktree, open a PR against dev, self-review it, auto-merge, tear down the branch + worktree, and pull latest main. Use when the user asks to work a go-live phase, tackle a checklist phase, or invokes /golive-phase.
---

# golive-phase

Execute a go-live phase from one of the role-focused roadmaps under `docs/GO_LIVE/` — either `DEV.md` (overall product, Phases 0–14) or `MOBILE.md` (Expo / React Native app, Phases M0–M5). Browse incomplete tasks, plan the work, implement in a dedicated worktree, open a PR, self-review it, auto-merge, then clean up and sync `main`.
...
Push the branch to **both** remotes per the project's push workflow — `origin` (the org repo, which the PR targets) and `backup` (the personal mirror):

```bash
git push -u origin feat/golive-<slug>-phase-<N-sanitised>
git push backup feat/golive-<slug>-phase-<N-sanitised>
```

If a `backup` remote is not configured, push to `origin` only and note it.
...
## Step 9 — Self-review before merging

Before merging, review your own diff and fix what you find. Do **not** merge a PR you have not reviewed.

1. Run the general review over the branch diff:
   ```
   /code-review
   ```
   Address every correctness finding it surfaces. Push fixups to the branch (both remotes) and re-run until the review is clean or the only remaining findings are explicitly out of scope.

2. **If the diff touches `auth/`, `guards/`, `jwt`, session management, role checks, payment webhooks, or any data-export flow**, additionally run:
   ```
   /security-review
   ```
   Per CLAUDE.md this is mandatory for those paths. Resolve any high/critical finding before proceeding — never merge a PR with an unaddressed security finding in sensitive code.

3. Sanity-check the gates locally (GitHub Actions may be paused for billing — verify locally rather than waiting on CI):
   ```bash
   moon run :lint
   moon run :test
   moon run api:typecheck   # for DEV phases touching the API
   moon run web:build       # for DEV phases touching web
   ```
   If a gate is red, fix the underlying code — do not weaken tests or skip assertions.

Summarise the self-review outcome (findings + how each was resolved) in a PR comment before merging.

---

## Step 10 — Auto-merge the PR

Once the self-review is clean and local gates pass, merge the PR into `dev`:

```bash
gh pr merge <pr-number-or-url> --squash --delete-branch
```

- Use `--squash` so the phase lands as one clean commit on `dev`.
- `--delete-branch` removes the remote branch on `origin`.
- If branch protection blocks the merge because required status checks did not run (CI paused for billing), retry with `--admin`:
  ```bash
  gh pr merge <pr-number-or-url> --squash --delete-branch --admin
  ```
- Do **not** use `--auto`: auto-merge waits for CI, which will not run.

Confirm the merge succeeded (`gh pr view <pr> --json state,mergedAt`).

---

## Step 11 — Tear down the branch and worktree

After the merge is confirmed:

```bash
# Remove the worktree (from the main working directory, not inside the worktree)
git worktree remove /tmp/ra-golive-<slug>-<N-sanitised>
git worktree prune

# Delete the local feature branch
git branch -D feat/golive-<slug>-phase-<N-sanitised>

# Clean the branch off the backup mirror too (origin was pruned by --delete-branch)
git push backup --delete feat/golive-<slug>-phase-<N-sanitised> 2>/dev/null || true
```

If `git worktree remove` refuses because of leftover build artifacts, retry with `--force`.

---

## Step 12 — Pull latest main

Return to the primary working directory, then sync `main` from `origin`:
...
This leaves the primary folder on an up-to-date `main`. (The phase work merged into `dev`; `main` receives it later via the `dev → test → main` promotion path — this step just makes sure the primary checkout is current.)

---

## Done

Print the following block (substitute `<doc>`, `<N>`, `<title>`, `<slug>`, `<pr-url>`):

```
Phase <N> — <title>  (from <doc>)
PR:       <pr-url>  ✅ merged into dev (squash)
Branch:   feat/golive-<slug>-phase-<N-sanitised>  — deleted (local + remotes)
Worktree: /tmp/ra-golive-<slug>-<N-sanitised>  — removed
Main:     primary folder synced to latest origin/main
```
